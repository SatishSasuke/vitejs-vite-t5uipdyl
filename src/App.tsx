...
export default function App() {
  ...
  // Function to handle the copy buttons
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-300 font-sans selection:bg-[#D4AF37] selection:text-black">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        @keyframes float3D {
          0% { transform: translateY(0px) rotateY(-15deg) rotateX(8deg); filter: drop-shadow(25px 25px 20px rgba(0,0,0,0.9)) drop-shadow(-10px -10px 20px rgba(212,175,55,0.15)) brightness(0.95) contrast(1.1); }
          50% { transform: translateY(-30px) rotateY(15deg) rotateX(-5deg); filter: drop-shadow(-25px 40px 30px rgba(0,0,0,0.9)) drop-shadow(15px 15px 30px rgba(212,175,55,0.4)) brightness(1.25) contrast(1.2); }
          100% { transform: translateY(0px) rotateY(-15deg) rotateX(8deg); filter: drop-shadow(25px 25px 20px rgba(0,0,0,0.9)) drop-shadow(-10px -10px 20px rgba(212,175,55,0.15)) brightness(0.95) contrast(1.1); }
        }
        .sculpture-container { perspective: 800px; transform-style: preserve-3d; }
        .animate-float { animation: float3D 8s ease-in-out infinite; transform-style: preserve-3d; will-change: transform, filter; }
        .bento-card {
        ...
        .bento-card:hover { border-color: rgba(212, 175, 55, 0.3); box-shadow: 0 10px 40px -10px rgba(212, 175, 55, 0.1), inset 0 1px 0 0 rgba(255, 255, 255, 0.05); }
      `}} />
      ...
      <nav className="fixed top-0 w-full z-[100] bg-[#050505] border-b border-[#D4AF37]/10 shadow-lg overflow-hidden">
        <div className="w-[95%] max-w-[1600px] mx-auto px-4 sm:px-8 h-16 sm:h-20 flex justify-center items-center">
          <div className="flex items-center relative text-center">
            ...
            <span 
              className="relative z-10 font-light tracking-[0.15em] sm:tracking-[0.3em] uppercase text-[9px] sm:text-[11px] text-white leading-[1.6] sm:leading-none cursor-pointer"
          ...
          <section className="relative min-h-[90vh] flex flex-col items-center justify-center pt-24 px-4 overflow-hidden">
            ...
            <div className="relative z-10 text-center flex flex-col items-center max-w-2xl">
              <h1 
                ...
                style={{ fontFamily: '"Cinzel", "Times New Roman", serif', color: '#ffffff', opacity: 1 }}
              >
              ...
              <div className="pt-4 flex flex-wrap justify-center gap-4">
                <a 
                  ...
                  className="inline-flex items-center justify-center gap-2 px-8 py-3.5 border border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition-all duration-500 font-medium tracking-[0.2em] uppercase text-xs group mb-4"
                >
                  View COA
                ...
                <a 
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full sm:w-auto px-8 py-3.5 rounded-none border border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition-all duration-500 font-medium tracking-[0.2em] uppercase text-xs flex items-center justify-center gap-2 group mb-4"
                >
          ...
          <main className="relative z-10 w-[95%] max-w-[1400px] mx-auto px-4 pb-24">
            <div className="bento-card rounded-2xl p-6 sm:p-8 w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 border-b border-white/5 pb-8 mb-6">
                <DetailItem icon={<Palette size={14} />} label="Artist" value="Raajashri Vegnishwar Nair" truncate={false} />
                <DetailItem icon={<Box size={14} />} label="Year" value="2026" />
                <DetailItem icon={<Layers size={14} />} label="Series" value="VoL IV | The Living Geode" truncate={false} />
                <div className="sm:col-span-2">
                  <DetailItem icon={<Diamond size={14} />} label="Type" value="Geological Surrealism Fine Art Sculpture" truncate={false} />
                </div>
                <DetailItem icon={<MapPin size={14} />} label="Size" value="1.3 x 1.9 ft" />
                <DetailItem icon={<Gem size={14} />} label="Weight" value="26kg" />
                <DetailItem icon={<ShieldCheck size={14} />} label="Edition" value="Master Piece 1/1" />
                <DetailItem icon={<Sparkles size={14} />} label="Digital Creation" value="No" />
                <div className="sm:col-span-2 md:col-span-3">
                  <DetailItem icon={<Fingerprint size={14} />} label="Signature" value="Signed on the Upper Right Hand Corner" truncate={false} />
                </div>
              </div>
            ...
            <div className="flex flex-col gap-4 w-full mb-10 mt-3 ">
              <div className="bento-card rounded-2xl p-6 sm:p-8 m ">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 ">
                  <DetailItem label="Art Registry ID" value={MOCK_DATA.artRegistryId} icon={<ShieldCheck size={14} />} onCopy={() => handleCopy(MOCK_DATA.artRegistryId, 'artRegistry')} isCopied={copiedField === 'artRegistry'} />
                  <DetailItem label="Art Serial number" value={MOCK_DATA.serialNumber} icon={<Layers size={14} />} onCopy={() => handleCopy(MOCK_DATA.serialNumber, 'serial')} isCopied={copiedField === 'serial'} />
                  <div className="sm:col-span-2">
                    <DetailItem label="Ledger Address (Smart Contract)" value={MOCK_DATA.ledgerAddress} icon={<LinkIcon size={14} />} onCopy={() => handleCopy(MOCK_DATA.ledgerAddress, 'ledger')} isCopied={copiedField === 'ledger'} truncate={false} />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bento-card rounded-2xl p-6">
                  ...
                  <div className="mt-4 pt-4 border-t border-white/5">
                    <DetailItem label="Token ID" value={MOCK_DATA.tokenId} icon={<Layers size={12} />} onCopy={() => handleCopy(MOCK_DATA.tokenId, 'token')} isCopied={copiedField === 'token'} />
                  </div>
                </div>
            ...
            <div className="max-w-md mx-auto bento-card rounded-2xl p-8 relative overflow-hidden group border border-[#D4AF37]/20 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
              ...
              <div className="relative z-10 flex flex-col items-center text-center">
                <span className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-2 flex items-center gap-2">
                  <Sparkles size={14} className="text-[#D4AF37]" /> Energy Matrix
                </span>
                <h4 className="text-lg text-white uppercase tracking-widest font-light mb-6">ATT Points Collected</h4>
          ...
          <section className="relative z-10 w-[95%] max-w-[1600px] mx-auto px-4 pb-32 mt-16 sm:mt-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 mb-20">
              <div className="flex flex-col group">
                <div className="relative h-[250px] sm:h-[500px] w-full overflow-hidden rounded-2xl bento-card border border-[#D4AF37]/20 transition-all duration-700 hover:border-[#D4AF37]/50 shadow-[0_10px_30px_rgba(0,0,0,0.5)] mb-8">
                  <div className="relative w-full h-full bg-[#111]">
                    <img src="/Paint1.png" alt="The Ecological | Herman WSY" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 opacity-80 group-hover:opacity-100" />
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-8 flex flex-col items-center text-center z-20 translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                    <h3 className="text-[#D4AF37] text-xl sm:text-2xl tracking-[0.2em] uppercase font-light mb-2" style={{ fontFamily: '"Cinzel", "Times New Roman", serif' }}>The Ecological</h3>
                    <div className="flex items-center gap-4 w-full justify-center">
                      <div className="h-[1px] w-8 bg-[#D4AF37]/30"></div>
                      <span className="text-gray-400 text-[10px] sm:text-xs tracking-[0.3em] uppercase font-medium">Herman WSY</span>
                      <div className="h-[1px] w-8 bg-[#D4AF37]/30"></div>
                    </div>
                  </div>
                </div>
                <div className="px-2">
                  <div className="mb-4 border-l-2 border-[#D4AF37] pl-4">
                    <h4 className="text-lg font-light text-white tracking-widest uppercase mb-1" style={{ fontFamily: '"Cinzel", "Times New Roman", serif' }}>The Biodiversity of The Hotsprings</h4>
              ...
              <div className="flex flex-col group">
                <div className="relative h-[250px] sm:h-[500px] w-full overflow-hidden rounded-2xl bento-card border border-[#D4AF37]/20 transition-all duration-700 hover:border-[#D4AF37]/50 shadow-[0_10px_30px_rgba(0,0,0,0.5)] mb-8">
                  <div className="relative w-full h-full bg-[#111]">
                    <img src="/Paint3.jpeg" alt="The Resonance | Garero Fumani" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 opacity-80 group-hover:opacity-100" />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#050505] via-[#050505]/40 to-transparent opacity-90 group-hover:opacity-70 transition-opacity duration-700"></div>
                  </div>
                  <div className="absolute bottom-0 left-0 w-full p-8 flex flex-col items-center text-center z-20 translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                    <h3 className="text-[#D4AF37] text-xl sm:text-2xl tracking-[0.2em] uppercase font-light mb-2" style={{ fontFamily: '"Cinzel", "Times New Roman", serif' }}>The Resonance</h3>
                    <div className="flex items-center gap-4 w-full justify-center">
                      <div className="h-[1px] w-8 bg-[#D4AF37]/30"></div>
                      <span className="text-gray-400 text-[10px] sm:text-xs tracking-[0.3em] uppercase font-medium">Garero Fumani</span>
                      <div className="h-[1px] w-8 bg-[#D4AF37]/30"></div>
                    </div>
                  </div>
                </div>
                <div className="px-2">
                  <div className="mb-4 border-l-2 border-[#D4AF37] pl-4">
                    <h4 className="text-lg font-light text-white tracking-widest uppercase mb-1" style={{ fontFamily: '"Cinzel", "Times New Roman", serif' }}>The Acoustic Geology of Fire</h4>
            ...
            <div className="mt-20 flex justify-center relative z-10">
              <button className="group relative flex items-center justify-center gap-6 px-12 py-5 cursor-pointer overflow-hidden transition-all duration-700">
                ...
                <svg className="relative z-10 w-4 h-4 text-[#D4AF37] group-hover:text-black transition-all duration-700 group-hover:translate-x-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {currentView === 'provenance' && (
        <div className="animate-in slide-in-from-right-8 duration-500 pt-24 pb-24 px-6 min-h-screen">
          <div className="max-w-6xl mx-auto">
            <button onClick={() => setCurrentView('main')} className="flex items-center gap-2 text-gray-400 hover:text-[#D4AF37] transition-colors uppercase tracking-widest text-xs font-semibold mb-12 group">
              <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Return to Registry
            </button>
            <div className="mb-16 text-center flex flex-col items-center">
              <h1 className="text-3xl sm:text-5xl font-light text-white tracking-widest uppercase mb-4" style={{ fontFamily: '"Cinzel", "Times New Roman", serif' }}>Lithic Codex</h1>
            ...
            <div className="mb-16 sm:mb-24">
              <div className="flex items-center gap-3 mb-3 border-b border-white/10 pb-4">
                ...
                <h2 className="text-2xl font-light text-white tracking-widest uppercase">The Vessel <span className="text-gray-600 mx-2">|</span> <span className="text-gray-400">Body</span></h2>
              </div>
            ...
            <div className="mb-24">
              <div className="flex items-center gap-3 mb-3 border-b border-[#D4AF37]/20 pb-4">
                ...
                <h2 className="text-2xl font-light text-white tracking-widest uppercase">The Spark <span className="text-[#D4AF37]/50 mx-2">|</span> <span className="text-[#D4AF37]">Soul</span></h2>
              </div>
              <p className="text-gray-400 text-sm font-light tracking-wider max-w-4xl mb-16 leading-[1.8]">
            ...
            <div className="mt-16 flex justify-center">
              <a href="https://www.gemsociety.org/article/gem-pricing-guide-sample/" target="_blank" rel="noopener noreferrer" className="px-8 py-3.5 rounded-none border border-[#D4AF37]/50 text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition-all duration-500 font-medium tracking-[0.2em] uppercase text-xs flex items-center justify-center gap-2 group">
                Learn More <ExternalLink size={14} className="group-hover:scale-110 transition-transform" />
              </a>
            </div>
          </div>
        </div>
      ...
      {/* PAGE 3: GEOGRAPHIC ORIGIN REGISTRY */}
        ...
        <div className="animate-in slide-in-from-right-8 duration-500 pt-24 pb-24 px-6 min-h-screen">
          <div className="max-w-6xl mx-auto">
            <button onClick={() => setCurrentView('main')} className="flex items-center gap-2 text-gray-400 hover:text-[#D4AF37] transition-colors uppercase tracking-widest text-xs font-semibold mb-12 group">
              ...
              <div className="mb-12 text-center flex flex-col items-center">
              <h1 className="text-3xl sm:text-5xl font-light text-white tracking-widest uppercase mb-4" style={{ fontFamily: '"Cinzel", "Times New Roman", serif' }}>Geographic Provenance</h1>
              <div className="h-px w-32 bg-gradient-to-r from-transparent via-[#D4AF37] to-transparent mb-8"></div>
              <p className="text-gray-400 max-w-2xl leading-relaxed font-light text-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              <div className="lg:col-span-8 bg-neutral-950/40 border border-[#D4AF37]/20 rounded-3xl p-4 sm:p-8 relative h-[450px] sm:h-[550px] flex items-center justify-center overflow-hidden group/map select-none">
                ...
                <img src="/map-ipoh.png" alt="Ipoh Perak Sourcing Map" className="absolute inset-0 w-full h-full object-cover opacity-30 mix-blend-luminosity group-hover/map:scale-105 transition-transform duration-1000 pointer-events-none" />
                <div className="absolute bottom-6 left-6 flex items-center gap-2 text-[10px] uppercase font-mono tracking-widest text-neutral-600">
                  <Compass size={16} className="animate-spin-slow text-neutral-700" /> REGIONAL MATRICES ACTIVE
                </div>
      ...
      <footer className="border-t border-white/10 py-16 text-center bg-[#000000] relative z-10">
        <div className="w-[95%] max-w-[1600px] mx-auto px-4 flex flex-col items-center">
          ...
          <p className="text-[10px] sm:text-xs text-gray-500 uppercase tracking-[0.2em] leading-loose mb-12 text-center max-w-2xl">
            ...
            <br />
            {/* ✨ NEW: Policy Links Added to Footer per standard requirements */}
            <span className="mt-4 block space-x-2">
               <a href="https://www.thefineartledger.com/privacy-policy" target="_blank" rel="noopener noreferrer" className="hover:text-[#D4AF37] transition-colors">Privacy Policy</a>[cite: 5]
               <span>|</span>
               <a href="https://www.thefineartledger.com/terms" target="_blank" rel="noopener noreferrer" className="hover:text-[#D4AF37] transition-colors">Terms & Conditions</a>[cite: 5]
            </span>
          </p>
          ...
          <div className="flex flex-wrap justify-center items-center gap-8 sm:gap-12 md:gap-16 w-full mt-8">
            {[
              ...
              <img 
                key={index} src={logo.src} alt={logo.alt} 
                className="h-10 sm:h-12 w-auto object-contain shrink-0 opacity-100 transition-transform duration-500 drop-shadow-[0_0_15px_rgba(255,255,255,0.5)] hover:scale-110 cursor-pointer"
              />
            ))}
          </div>
        </div>
